...
(function() {
  ...
  function updateStats() {
    ...
    for (const town of Object.keys(prospects)) {
      for (const p of prospects[town]) {
        const status = p.status || 'new';
        if (status === 'new') newCount++;
        else if (status === 'contacted' || status === 'responded') contactedCount++;
        else if (status === 'converted') convertedCount++;
      }
    }

    const conversionRate = contactedCount > 0
      ? ((convertedCount / contactedCount) * 100).toFixed(0)
      : 0;

    if (statNewEl) statNewEl.textContent = newCount;
    if (statContactedEl) statContactedEl.textContent = contactedCount;
    if (statConvertedEl) statConvertedEl.textContent = convertedCount;
    if (statRateEl) statRateEl.textContent = `${conversionRate}%`;
  }
  ...
  function createTownCard(town, townProspects) {
    ...
    const contactedCount = townProspects.filter(p => p.status === 'contacted' || p.status === 'responded').length;
  ...
  function createProspectItem(prospect) {
    ...
    const hasSocial = (prospect.facebook && prospect.facebook !== 'Not available') ||
                      (prospect.instagram && prospect.instagram !== 'Not available');
    const isHot = hasMobile && hasSocial;
    const hotIndicator = isHot ? '<span style="color: #ff6b35; margin-left: 4px;">🔥</span>' : '';

    return `
      <button class="prospect-item ${statusClass}">
        <span class="prospect-name">${escapeHtml(prospect.businessName)}${hotIndicator}</span>
        ${flagged}
  ...
  function setupEventListeners() {
    ...
    if (viewAnalyticsBtn) viewAnalyticsBtn.addEventListener('click', () => openPage('analytics.html'));
