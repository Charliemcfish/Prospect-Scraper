...
(function() {
   ...
   * Only flags if the business name appears in the actual domain (not path)
   * Excludes directory/review sites like mybuilder.com, yell.com, etc.
   */
  function findPotentialWebsite(businessName) {
    // List of directory/review sites to exclude
    const directorySites = [
      'mybuilder.com', 'yell.com', 'checkatrade.com', 'trustatrader.com',
      'ratedpeople.com', 'bark.com', 'freeindex.co.uk', 'yelp.com',
      'trustpilot.com', 'reviews.co.uk', 'houzz.com', 'houzz.co.uk',
      '192.com', 'cylex-uk.co.uk', 'hotfrog.co.uk', 'brownbook.net',
      'scoot.co.uk', 'thebestof.co.uk', 'thomsonlocal.com', 'misterwhat.co.uk',
      'businessmagnet.co.uk', 'finder.com', 'nextdoor.co.uk', 'nextdoor.com',
      'facebook.com', 'instagram.com', 'twitter.com', 'x.com',
      'linkedin.com', 'youtube.com', 'tiktok.com', 'pinterest.com',
      'google.com', 'maps.google', 'goo.gl', 'bit.ly',
      'which.co.uk', 'local.google.com', 'tripadvisor',
      'apple.com', 'bing.com', 'wikipedia.org', 'gov.uk'
    ];

    // Clean business name for matching - extract key words
    ...
    const commonWords = ['the', 'and', 'for', 'ltd', 'limited', 'services', 'service', 'company', 'plumbing', 'heating', 'electrical', 'building', 'builders', 'plumber', 'plumbers', 'electrician', 'electricians'];
    const nameWords = businessNameLower.split(/\s+/)
      .filter(w => w.length > 2 && !commonWords.includes(w));

    // Look for web links in the sidebar (more accurate than web results)
    const sidebarLinks = document.querySelectorAll('[data-attrid*="kc:/"] a[href], .OOijTb a[href], .PZPZlf a[href]');

    for (const link of sidebarLinks) {
      const href = link.href || '';

      // Skip social and directory sites
      const isDirectory = directorySites.some(site => href.includes(site));
      ...
      try {
        ...
        // Check if any significant business name word appears in the domain itself
        // This ensures we only flag actual business websites, not directory listings
        const domainHasBusinessName = nameWords.some(word => {
          // Word must be at least 4 chars to avoid false positives
          if (word.length < 4) return false;
          // Remove common suffixes from domain for matching
          const cleanDomain = domain.replace(/\.(co\.uk|com|uk|org|net)$/, '');
          return cleanDomain.includes(word);
        });

        if (domainHasBusinessName) {
          return link.href;
        }
      } catch (e) {
        // Invalid URL, skip
   ...
   * Using faster timing (0.5-1s instead of 3s)
   */
  async function clickAndWaitForSidebar(businessCard) {
    ...
    if (clickable) {
      ...
      // Wait for sidebar to load - using faster timing
      await wait(500); // Reduced initial wait

      // Wait for address or other detail elements
      await waitForElement('[data-local-attribute="d3adr"]', 1000);
      await wait(300); // Reduced extra wait
    }
  }
  ...
  async function scanPage() {
    ...
    try {
      ...
      // Save prospects (with global duplicate prevention)
      const { addedCount, duplicateCount } = await saveProspects(prospects, location);
      ...
      let message = `Added ${addedCount} new prospects in ${location}`;
      if (duplicateCount > 0) {
        message += ` (${duplicateCount} duplicates skipped)`;
      }
      if (flaggedProspects.length > 0) {
        message += ` - ${flaggedProspects.length} flagged`;
      }
      showNotification(message, 'success');
      updateButtonState('ready', addedCount);
  ...
  /**
   * Check if a prospect already exists in ANY location (global duplicate check)
   */
  function isGlobalDuplicate(prospect, allProspects) {
    for (const town of Object.keys(allProspects)) {
      for (const existingProspect of allProspects[town]) {
        // Check by business name (case insensitive)
        if (existingProspect.businessName.toLowerCase() === prospect.businessName.toLowerCase()) {
          return true;
        }
        // Check by phone number (if available)
        if (prospect.phone !== 'Not available' &&
            existingProspect.phone !== 'Not available' &&
            existingProspect.phone === prospect.phone) {
          return true;
        }
      }
    }
   ...
   * With global duplicate prevention across all locations
   */
  async function saveProspects(newProspects, location) {
    ...
    // Add new prospects, avoiding duplicates across ALL locations
    for (const prospect of newProspects) {
      // Check for global duplicates (across all towns/locations)
      if (isGlobalDuplicate(prospect, allProspects)) {
        duplicateCount++;
        continue;
      }
    ...
    // Return counts for notification
