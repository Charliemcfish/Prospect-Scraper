 ...
 * Shows only prospects with status 'contacted'
 */
...
(function() {
  ...
  async function loadProspects() {
    try {
      ...
      // Filter to only contacted prospects (not converted)
      contactedProspects = {};
      for (const town of Object.keys(prospects)) {
        const contacted = prospects[town].filter(p => p.status === 'contacted');
  ...
  function updateStats() {
    ...
    for (const town of Object.keys(contactedProspects)) {
      total += contactedProspects[town].length;
    }

    totalStatsEl.innerHTML = `<span class="stat-value">${total}</span> contacted prospect${total !== 1 ? 's' : ''}`;
  }
  ...
  function createProspectCard(prospect, town) {
    ...
    return `
      <article class="prospect-card status-contacted" data-id="${prospect.id}">
        <header class="card-header">
          ...
          <div class="card-actions">
            <button class="btn btn-sm btn-primary convert-btn" data-id="${prospect.id}">
